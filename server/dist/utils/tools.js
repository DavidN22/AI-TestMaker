...
export const customSelectTool = tool({
    ...
    description: "Run a safe, read-only SQL SELECT query on the devtests table for the current user.",
    parameters: z.object({
        query: z
            .string()
            .min(1)
            .describe("A SQL SELECT query scoped to the user. Must include a WHERE clause using userId."),
    }),
    async execute({ query }) {
