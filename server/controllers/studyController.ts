...
const studyController = {
  ...
  generateFlashcards: async (req: Request, res: Response, next: NextFunction) => {
...
- ONLY respond with valid JSON, no additional text`;
  ...
  generateFlashcardsFromTest: async (req: Request, res: Response, next: NextFunction) => {
        ...
        const questionsContext = quiz_data
          .map((q: any, idx: number) => {
            const correctAnswers = Array.isArray(q.correct_answer)
              ? q.correct_answer.map((ans: string) => q.answers[ans]).join(", ")
...
- ONLY respond with valid JSON, no additional text`;
  ...
  studyChatbot: async (req: Request, res: Response, next: NextFunction) => {
