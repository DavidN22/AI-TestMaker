interface TestSidebarProps {
    questions: { user_answer?: string | string[]; select_two?: boolean }[]; // Add select_two to props
    currentIndex: number;
    setCurrentIndex: (index: number) => void;
    timer: number;
  }
  
  export default function TestSidebar({ questions, currentIndex, setCurrentIndex, timer }: TestSidebarProps) {
    ...
    const isQuestionAnswered = (user_answer?: string | string[], select_two?: boolean) => {
      if (select_two) {
    ...
    const getButtonClass = (index: number, question: { user_answer?: string | string[]; select_two?: boolean }) => {
      if (index === currentIndex) {
        return "bg-gray-700 text-white";
      } else if (question.select_two && Array.isArray(question.user_answer) && question.user_answer.length === 1) {
    ...
    return (
      <aside className="w-80 bg-white dark:bg-[#1E1E1E] p-6 shadow-lg border border-gray-300 dark:border-gray-700 transition-all flex flex-col justify-between">
        {/* Header */}
        <div>
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">AWS Cloud Practitioner</h2>
          <p className="text-gray-600 dark:text-gray-400 text-sm">Basic AWS knowledge assessment</p>
          ...
          <div className="grid grid-cols-5 gap-1 mt-6">
            {questions.map((q, index) => (
               ...
               className={`p-3 text-sm font-medium rounded-md transition ${getButtonClass(index, q)}`}
             >
               {index + 1}
             </button>
            ))}
        ...
        {/* Bottom Buttons */}
        <div className="mt-8">
          <button className="w-full bg-red-600 text-white py-2 rounded-lg hover:bg-red-500">
            Cancel Test
          </button>
          <button className="w-full bg-green-600 text-white py-2 rounded-lg hover:bg-green-500 mt-3">
            Submit Test
          </button>
        </div>
      </aside>
