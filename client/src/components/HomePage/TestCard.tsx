...
export default function TestCard({ title, description }: TestCardProps) {
  ...
  return (
    <div
      ...
      dark:hover:border-blue-400 cursor-pointer flex flex-col min-h-[220px] space-y-3 group"
      onClick={() => setIsModalOpen(true)}
    >
      {/* Title & Icon */}
      <div className="flex items-center gap-3 min-w-0">
        <span className="text-2xl">{getEmoji(title)}</span>
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white tracking-wide truncate group-hover:whitespace-normal group-hover:overflow-visible">
          {title}
        </h2>
      </div>
